...
class ConfigMenu(customtkinter.CTkFrame):
    def __init__(self, master, response_selection=None, on_save=None):
        ...
        self.treeview.heading("brainwave", text="Brainwave", anchor="center")
        self.treeview.heading("panel", text="Panel", anchor="center")
        self.treeview.heading("target", text="Target", anchor="center")
        self.treeview.heading("led", text="LED", anchor="center")
        self.treeview.heading("audio", text="Audio", anchor="center")
        self.treeview.heading("motor", text="Motor", anchor="center")

        self.treeview.column("brainwave", width=125, anchor="center", stretch=True)
        self.treeview.column("panel", width=75, anchor="center", stretch=True)
        self.treeview.column("target", width=75, anchor="center", stretch=True)
        self.treeview.column("led", width=90, anchor="center", stretch=True)
        self.treeview.column("audio", width=75, anchor="center", stretch=True)
        self.treeview.column("motor", width=150, anchor="center", stretch=True)

        self.treeview.grid(row=5, column=0, columnspan=3, padx=(25,0), pady=10, sticky="nsew")

        self.treeview.bind("<Control-r>", lambda e: self.remove_row())

        self.treeview.insert("", "end", values=("Alpha 8-12 [Hz]", "Panel 1", "Dancer 1" , "c:#ff0000 f:255"))
        self.treeview.insert("", "end", values=("Gamma 30-50[Hz]", "", "Synch", "", "C_sharp.wav", ""))
        self.treeview.insert("", "end", values=("Gamma 30-50[Hz]", "Panel 5", "Synch", "", "", "a:5000 s:100 d:down h:3"))
...
class MotorCtrl(customtkinter.CTkFrame):
    def __init__(self, master, on_run=None, on_emergency=None):
        ...
        # -------------------- Panel Height Entry --------------------
        self.height_label = customtkinter.CTkLabel(self, text="Panel Extended Height:", fg_color="transparent", font=("Open Sans", 14))
        self.height_label.grid(row=5, column=0, padx=5, pady=0, sticky="n")
        self.height_entry = customtkinter.CTkEntry(self, placeholder_text="Extended height in feet", width=150, height=20)
        self.height_entry.grid(row=5, column=0, padx=(35,0), pady=(30,0), sticky="nw")
...
class ConfigMode(customtkinter.CTkFrame):
    def __init__(self, master, on_save=None):
        ...
        self.motor_frame.height_label.configure(width=140)
        self.motor_frame.height_entry.configure(width=145)
        self.motor_frame.height_label.grid(row=4, column=1, columnspan=2, padx=(100,50), pady=(10,0), sticky="nw")
        self.motor_frame.height_entry.grid(row=4, column=1, columnspan=2, padx=(100,50), pady=(40,0), sticky="nw")
